...
def gloveFeature(headlines, bodies):
    X = []
...
def makeFeatureVec(title , content):
    ...
    for t in doc_title:
        try:
             titleVec = np.add(titleVec,t.vector)
    ...
    for c in doc_content:
        try:
             contentVec = np.add(contentVec,c.vector)
        except KeyError as e:
            continue
        content_count += 1
...
def generate_features(stances,dataset,name):
    ...
    X = gen_or_load_feats(gloveFeature, h, b, "features/gloveFeature."+name+".npy")
